...
import com.daml.platform.store.appendonlydao.events.BufferedTransactionsReaderSpec.transactionLogUpdate
...
class BufferedTransactionsReaderSpec
  ...
  "getTransactions" when {
    ...
    val offsetUpdates @ Seq(
      ...
      (1 to 4).map { idx =>
        Offset.fromByteArray(BigInt(idx + 1234).toByteArray) -> transactionLogUpdate(s"tx-$idx")
      }
